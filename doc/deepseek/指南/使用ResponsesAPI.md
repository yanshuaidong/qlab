...
> 来源：[https://api-docs.deepseek.com/zh-cn/guides/responses_api](https://api-docs.deepseek.com/zh-cn/guides/responses_api)
...
请参考[接入 Codex](../快速开始/接入工具/接入Codex.md)
...
Responses API 支持使用 `deepseek-flash` 模型传入图片，适用的图片限制与支持格式与[对话补全](图像理解.md#limits)一致。

图片通过 `message` item 中的 `input_image` 内容块提供，使用 `image_url`（`http(s)` URL 或 base64 data URL）或 `file_id`（通过 [Files API](文件API.md) 上传的图片）二者之一：
...
- `file_id`：通过 [Files API](文件API.md) 上传的图片文件 ID，形如 `file-api-...`。
...
- 与对话补全相同的图片限制（内联单张 32 MiB、`file_id` 单张 64 MiB、不含 `file_id` 图片总计 64 MiB，含 `file_id` 图片最高 200 MiB、单请求 600 张等）同样适用，详见[图像理解：限制](图像理解.md#limits)。
...
| 参数 | 支持情况 |
| --- | --- |
| `model` | 支持。`deepseek-flash`，见[模型 & 价格](../快速开始/模型与价格.md) |
...
| `user` | 支持。参考[限速与用户隔离](../快速开始/限速与隔离.md) |
...
| `prompt_cache_key` / `prompt_cache_retention` | 不支持。上下文缓存自动管理，见[上下文硬盘缓存](上下文硬盘缓存.md) |
...
- `input_tokens`：输入 token 数，其中 `input_tokens_details.cached_tokens` 为命中[上下文缓存](上下文硬盘缓存.md)的 token 数
