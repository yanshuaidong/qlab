...
category: 美股数据
type: api
source: https://tushare.pro/document/2?doc_id=252
markdown: https://tushare.pro/wctapi/documents/252.md
updated: 2026-09-15
api: us_basic
---
...
> **接口：** `us_basic` · [官方文档](https://tushare.pro/document/2?doc_id=252)
...
接口：us_basic
描述：获取美股列表信息
限量：单次最大6000，可分页提取。覆盖的证券类型：ADR-美国存托凭证；GDR-全球存托凭证；EQ-普通股；PF-优先股（本次新增）。
**注：美股代码存在代码复用，变更，多市场交易等，可能存在数据重复情况，需要加入上市状态判断数据情况**
...
| list_stauts | str | N | 上市状态 | L上市D退市P暂停上市 |
